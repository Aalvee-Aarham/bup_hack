...
> **OPTIMIZATION SCORE:** `min(1, organizer_optimal_cost / recalculated_team_cost)`. Optimization Quality = 10 x the average `quality_ratio` across all optimization hidden cases. If `organizer_optimal_cost` and `recalculated_team_cost` are both within numeric tolerance of 0, `quality_ratio = 1`. If `organizer_optimal_cost` is within tolerance of 0 but team cost is above tolerance, `quality_ratio` is scored accordingly (approaches 0).
